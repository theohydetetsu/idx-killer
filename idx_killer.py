...
# ==========================================
CACHE_FILE = "jihan_ghina_saham_cache_v19_3.json"
...
# ==========================================
# 1. LUXURY UI & COMFORT EYE-CARE CSS (V19.3)
# ==========================================
st.set_page_config(page_title="JIHAN-GHINA Ultimate V19.3 + Fibo", page_icon="✨", layout="wide")
...
<style>
    ...
    /* Tabs Overhaul - Comfort Classic Style */
...
# ==========================================
# 2. CORE ENGINE & INDICATORS + FIBONACCI
# ==========================================
MASTER_UNIVERSE = [
...
def hitung_fibonacci(df, window=60):
    """Engine Fibonacci Retracement berdasarkan Swing High & Low (Default 60 Hari)"""
    df_slice = df.tail(window)
    if df_slice.empty: return {}
    swing_high = float(df_slice['High'].max())
    swing_low = float(df_slice['Low'].min())
    diff = swing_high - swing_low
    
    return {
        "swing_high": swing_high,
        "swing_low": swing_low,
        "fib_0": swing_high,          # 0.0% (Puncak)
        "fib_236": swing_high - (0.236 * diff),
        "fib_382": swing_high - (0.382 * diff),
        "fib_500": swing_high - (0.500 * diff),
        "fib_618": swing_high - (0.618 * diff), # Golden Ratio
        "fib_786": swing_high - (0.786 * diff),
        "fib_100": swing_low          # 100.0% (Dasar)
    }
...
def fetch_single_stock(emiten, mode_tf):
    try:
        ...
        vol_sma20 = float(df['Volume'].rolling(window=20).mean().iloc[-1])
        ...
        # Hitung Fibonacci
        fibo_levels = hitung_fibonacci(df, window=60)
...
with st.sidebar:
    ...
    st.markdown("<p style='color:#71717A; font-size:9px; letter-spacing:1px; margin-bottom:20px;'>EDITION V19.3 + FIBO</p>", unsafe_allow_html=True)
    ...
    if st.button("🔄 SCAN", use_container_width=True):
        ...
        radar_bar = st.progress(0, text="Membaca Market & Fibo...")
...
# ==========================================
# 4. MAIN LAYOUT (RESTORED CLASSIC TABS + FIBO)
# ==========================================
if not st.session_state.raw_stocks:
    ...
    with tab_scan:
        ...
        if s:
            ...
            # --- FIBONACCI RETRACEMENT ENGINE CARD ---
            fibo = s.get('FIBO', {})
            if fibo:
                st.markdown(f"""
                <div class="pro-card">
                    <div class="card-label">📐 FIBONACCI RETRACEMENT (60-DAY SWING)</div>
                    <div class="data-grid" style="grid-template-columns: repeat(4, 1fr); margin-bottom:10px;">
                        <div><span class="data-label">SWING HIGH (0.0%)</span><span class="data-value" style="color:#EF4444;">Rp {int(fibo.get('swing_high', 0)):,}</span></div>
                        <div><span class="data-label">FIBO 23.6%</span><span class="data-value">Rp {int(fibo.get('fib_236', 0)):,}</span></div>
                        <div><span class="data-label">FIBO 38.2%</span><span class="data-value">Rp {int(fibo.get('fib_382', 0)):,}</span></div>
                        <div><span class="data-label">FIBO 50.0%</span><span class="data-value">Rp {int(fibo.get('fib_500', 0)):,}</span></div>
                    </div>
                    <div class="data-grid" style="grid-template-columns: repeat(4, 1fr);">
                        <div><span class="data-label">FIBO 61.8% (GOLDEN)</span><span class="data-value" style="color:#10B981;">Rp {int(fibo.get('fib_618', 0)):,}</span></div>
                        <div><span class="data-label">FIBO 78.6%</span><span class="data-value">Rp {int(fibo.get('fib_786', 0)):,}</span></div>
                        <div><span class="data-label">SWING LOW (100%)</span><span class="data-value" style="color:#3B82F6;">Rp {int(fibo.get('swing_low', 0)):,}</span></div>
                        <div><span class="data-label">STATUS LEVEL</span><span class="data-value" style="color:#C6A87C;">Area Pantul Dinamis</span></div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
    ...
    with tab_export:
        ...
        if not df_all.empty:
            ...
            st.download_button(label="📥 DOWNLOAD CSV (EXCEL)", data=csv_data, file_name=f"JG_Ultimate_V19_3_Fibo_Watchlist.csv", mime="text/csv", use_container_width=True)

    # ------------------------------------------
    # TAB 3: SOP & RISK CALCULATOR
    # ------------------------------------------
    with tab_sop:
        st.markdown("""
        <div class="pro-card">
            <h4 style="color:#C6A87C; margin-top:0;">📖 SOP TRADING AMAN + FIBO</h4>
            <ol style="color:#A1A1AA; font-size:13px; padding-left:15px; line-height:1.6;">
                ...
                <li><b>Gunakan Fibonacci 61.8%:</b> Perhatikan level Golden Ratio Fibonacci sebagai area akumulasi / support kuat saat harga koreksi sehat.</li>
                ...
                <li><b>Wajib Disiplin:</b> Pasang Auto-Order di aplikasi sekuritas Anda tepat di angka <span style="color:#EF4444; font-weight:bold;">TRAILING STOP / CUT LOSS</span>.</li>
            </ol>
        </div>
        """, unsafe_allow_html=True)
