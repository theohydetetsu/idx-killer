...
# ==========================================
# 3. CORE ENGINE DATA FETCHING & INDICATORS
# ==========================================
MASTER_UNIVERSE = [
...
def hitung_squeeze(df, period=20):
...
def fetch_single_stock(emiten, mode_tf):
    try:
        ...
        squeeze_status = "🔥 SQUEEZE ON" if is_squeeze else "➖ LEPAS"

        # --- ALPHA 20D LOGIC ---
        try:
            ret_20d = ((harga_skg - float(df['Close'].iloc[-20])) / float(df['Close'].iloc[-20])) * 100
        except:
            ret_20d = 0.0

        # --- VPT DIVERGENCE LOGIC ---
        recent_20 = df.tail(20)
        try:
            price_low_idx = recent_20['Close'].argmin()
            vpt_low_idx = recent_20['VPT'].argmin()
            vpt_div = "🟢 SMART ACCUM" if vpt_low_idx < price_low_idx else "➖ NORMAL"
        except:
            vpt_div = "➖ NORMAL"
            ...
            if raw_target:
                if is_trading_mode:
                    ...
                    if "JACKPOT" in setup_grade:
                        ...
                        desc = "⚡ <b>MOMENTUM GORENGAN CEPAT:</b> Bandar sedang injak gas. Squeeze Alert mungkin sedang aktif. Cocok untuk tektok harian!"
                        risk_multiplier = 1.5
                    else:
...
if not st.session_state.scan_clicked or not st.session_state.raw_stocks:
    ...
    if "TRADING" in engine_mode:
        ...
        def style_trading(row):
            styles = []
            if 'JACKPOT' in str(row['REKOMENDASI']): bg_rek = 'background-color: rgba(250, 204, 21, 0.2); color: #facc15; font-weight:900;'
            elif 'A+' in str(row['REKOMENDASI']): bg_rek = 'background-color: rgba(16, 185, 129, 0.15); color: #10b981; font-weight:900;'
            elif 'AGGRESSIVE' in str(row['REKOMENDASI']): bg_rek = 'background-color: rgba(139, 92, 246, 0.15); color: #c4b5fd; font-weight:900;'
            elif 'B' in str(row['REKOMENDASI']): bg_rek = 'background-color: rgba(56, 189, 248, 0.12); color: #38bdf8;'
        ...
        with tab1:
            st.markdown("<h4 style='color:#facc15; font-weight:900;'>💥 RADAR SEROK BAWAH & BULLISH DIVERGENCE</h4>", unsafe_allow_html=True)
            if df_jackpot.empty:
                st.info("💡 Belum ada emiten yang terdeteksi masuk area Serok Bawah / Bullish Divergence pada pemindaian ini.")
            else:
                st.dataframe(df_jackpot.style.apply(style_trading, axis=1), use_container_width=True, height=400)
                excel_jackpot = export_df_to_excel_buffer(df_jackpot, st.session_state.last_update, "Target_Serok_Bawah")
                st.download_button(
                    label="📥 Export Target Serok Bawah (Excel)",
                    data=excel_jackpot,
                    file_name=f"Jackpot_Serok_Bawah_{file_timestamp}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                )

        with tab2:
            st.markdown("<h4 style='color:#00f2fe; font-weight:900;'>🚀 TOP 15 MOMENTUM & ACCUMULATION LEADERBOARD</h4>", unsafe_allow_html=True)
            if df_trading.empty:
                st.warning("Data trading kosong.")
            else:
                st.dataframe(df_trading.head(15).style.apply(style_trading, axis=1), use_container_width=True, height=480)
                excel_trading = export_df_to_excel_buffer(df_trading, st.session_state.last_update, "Trading_Signal")
                st.download_button(
                    label="📥 Export Top Momentum (Excel)",
                    data=excel_trading,
                    file_name=f"Trading_Signal_{file_timestamp}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                )

        with tab3:
            st.markdown("""
            ### 📜 STANDARD OPERATING PROCEDURE (SOP) TRADING V17.0
            
            1. **Prioritas Sinyal Jackpot (Serok Bawah):**
               * **Bullish Divergence:** Terjadi ketika harga membentuk *Lower Low* namun indikator RSI membentuk *Higher Low*. Sinyal pembalikan terkuat.
               * **Whale Bottom Absorption:** Lonjakan volume signifikan di area *support* dengan ekor bawah panjang (pinbar). Smart Money sedang menyerap penawaran retail.
               * **Oversold Rebound:** Stochastic K memotong D naik dari area < 30 di dekat dasar 20 hari.
            2. **Aturan Entry & Sizing:**
               * Lakukan entri bertahap di kisaran **AREA BELI**.
               * Alokasikan lot sesuai kolom **REKOMENDASI LOT** untuk menjaga batas risiko kerugian maksimal per transaksi.
            3. **Disiplin Trailing Stop & Risk Management:**
               * Selalu tempatkan order Stop Loss / Trailing Stop pada level **TRAILING STOP** yang telah dihitung otomatis oleh *Chandelier Exit Engine*.
               * Bila IHSG berstatus *RISK OFF (BEARISH)*, ukuran posisi otomatis dipangkas 50% oleh sistem (*Auto-Brake*).
            """)

        render_cross_validation_ui(top_trading_tickers, climate_mult, is_trading_mode=True)

    else:
        tab1, tab2, tab3 = st.tabs(["🏛️ TOP VALUE & DIVIDEND", "🌌 CLUSTERING INVESTASI", "📜 SOP INVESTMENT"])

        def style_invest(row):
            styles = []
            for c, val in row.items():
                if c == 'VALUASI':
                    if 'UNDERVALUED' in str(val): styles.append('background-color: rgba(0, 242, 254, 0.15); color: #00f2fe; font-weight:900;')
                    elif 'FAIR' in str(val): styles.append('background-color: rgba(16, 185, 129, 0.15); color: #10b981; font-weight:900;')
                    else: styles.append('background-color: rgba(244, 63, 94, 0.12); color: #f43f5e; font-weight:800;')
                elif c == 'DIV YIELD (%)':
                    styles.append('color: #10b981; font-weight: 900; text-align:center;')
                elif c in ['PER (x)', 'PBV (x)', 'PEG (x)']:
                    styles.append('color: #38bdf8; font-weight: 800; text-align:center;')
                else:
                    styles.append('color: #f8fafc;')
            return styles

        with tab1:
            st.markdown("<h4 style='color:#10b981; font-weight:900;'>💎 TOP 15 VALUE & HIGH DIVIDEND EMITEN</h4>", unsafe_allow_html=True)
            if df_invest.empty:
                st.warning("Data investasi kosong.")
            else:
                st.dataframe(df_invest.head(15).style.apply(style_invest, axis=1), use_container_width=True, height=480)
                excel_invest = export_df_to_excel_buffer(df_invest, st.session_state.last_update, "Investment_Signal")
                st.download_button(
                    label="📥 Export Investment Matrix (Excel)",
                    data=excel_invest,
                    file_name=f"Investment_Matrix_{file_timestamp}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                )

        with tab2:
            st.markdown("<h4 style='color:#00f2fe; font-weight:900;'>🌌 KELOMPOK CLUSTERING PORTOFOLIO</h4>", unsafe_allow_html=True)
            col_c1, col_c2, col_c3 = st.columns(3)
            
            with col_c1:
                st.markdown("<div style='background:rgba(15,23,42,0.8); border:1px solid rgba(0,242,254,0.3); padding:15px; border-radius:12px;'><h5 style='color:#00f2fe; margin-top:0;'>💎 Deep Value Bargain</h5><p style='font-size:0.75rem; color:#94a3b8;'>PER &lt; 10x dan PBV &lt; 1.0x (Sangat Murah)</p>"+render_badges(cluster_deep_value, "#00f2fe")+"</div>", unsafe_allow_html=True)
                
            with col_c2:
                st.markdown("<div style='background:rgba(15,23,42,0.8); border:1px solid rgba(250,204,21,0.3); padding:15px; border-radius:12px;'><h5 style='color:#facc15; margin-top:0;'>🚀 High Growth (Undervalued)</h5><p style='font-size:0.75rem; color:#94a3b8;'>PEG Ratio 0.0 - 1.0x (Pertumbuhan Tinggi Harga Murah)</p>"+render_badges(cluster_high_growth, "#facc15")+"</div>", unsafe_allow_html=True)
                
            with col_c3:
                st.markdown("<div style='background:rgba(15,23,42,0.8); border:1px solid rgba(16,185,129,0.3); padding:15px; border-radius:12px;'><h5 style='color:#10b981; margin-top:0;'>👑 Dividend Kings</h5><p style='font-size:0.75rem; color:#94a3b8;'>Dividend Yield &ge; 5.0% per Tahun</p>"+render_badges(cluster_div_kings, "#10b981")+"</div>", unsafe_allow_html=True)

        with tab3:
            st.markdown("""
            ### 📜 STANDARD OPERATING PROCEDURE (SOP) INVESTMENT
            
            1. **Kriteria Seleksi Permata Tersembunyi (Undervalued):**
               * **Valuasi Rendah:** PER < 15x dan PBV < 1.5x.
               * **Pertumbuhan Sehat:** PEG Ratio di rentang 0.1 hingga 1.0x menandakan laba tumbuh lebih cepat dibanding kenaikan harga saham.
               * **Imbal Hasil Dividen:** Yield di atas suku bunga acuan (> 4.0%) memberikan perlindungan arus kas.
            2. **Strategi Akumulasi:**
               * Lakukan cicil beli dengan metode **Dollar Cost Averaging (DCA)** bulanan atau saat terjadi koreksi teknikal mendalam (*Pullback*).
               * Evaluasi kembali kinerja fundamental perusahaan setiap laporan keuangan triwulanan rilis.
            """)
...
# ==========================================
st.markdown("---")
st.markdown("<div style='text-align: center; color: #64748b; font-size: 0.75rem; margin-bottom: 20px;'>JIHAN-GHINA Ultimate v17.0 Jackpot Edition • Powered by Quantitative Matrix Engine</div>", unsafe_allow_html=True)
